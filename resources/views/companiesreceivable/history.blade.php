    ...
    <table class="table table-light caption-top table-bordered table-hover">
        ...
        <thead class="thead-light">
            <tr>
                ...
                <th colspan="2" class="text-center">Pendiente Cobrar</th>
            </tr>
            <tr>
        ...
        <tbody>
            @foreach($grupofacturas as $monthYear => $statuses)
                <tr>
                    ...
                    <td class="text-center">{{ $statuses['pendiente_facturar'] > 0 ? '$' . number_format($statuses['pendiente_facturar'], 2) : '-' }}</td>
    
                    {{-- Pagado --}}
                    <td class="text-center">{{ $monthYear }}</td>
                    <td class="text-center">{{ $statuses['pagado'] > 0 ? '$' . number_format($statuses['pagado'], 2) : '-' }}</td>
    
                    {{-- Pendiente Cobrar --}}
                    <td class="text-center">{{ $monthYear }}</td>
                    <td class="text-center">{{ $statuses['pendiente_cobrar'] > 0 ? '$' . number_format($statuses['pendiente_cobrar'], 2) : '-' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
