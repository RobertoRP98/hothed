...
<div class="row mb-4">
  <div class="col-md-4">
    <div class="form-outline">
      <select class="form-select" name="status_requisition" id="status_requisition" disabled>
        <option value="Pendiente" {{old('status_requisition',isset($requisition) && $requisition->status_requisition == 'Pendiente' ? 'selected':'')}}>PENDIENTE DE AUTORIZACIÓN</option>
  ...
  <div class="col-md-4">
    <div class="form-outline">
      <select class="form-select" name="importance" id="importance" disabled>
        <option value="Baja" {{old('importance',isset($requisition) && $requisition->importance == 'Baja' ? 'selected':'')}}>BAJA</option>
        <option value="Media" {{old('importance',isset($requisition) && $requisition->importance == 'Media' ? 'selected':'')}}>MEDIA</option>
        <option value="Alta" {{old('importance',isset($requisition) && $requisition->importance == 'Alta' ? 'selected':'')}}>ALTA</option>
      </select>
  ...
  <div class="col-md-4">
    <div class="form-outline">
      <select id="finished" name="finished" class="form-select" disabled>
        <option value="0" {{ old('finished', isset($requisition) ? $requisition->finished : 0) == 0 ? 'selected' : '' }}>NO</option>
        <option value="1" {{ old('finished', isset($requisition) ? $requisition->finished : 0) == 1 ? 'selected' : '' }}>SI</option>
      </select>
      <label class="form-label" for="finished">¿REQUISICIÓN FINALIZADA?</label>
    </div>
  </div>
</div>
...
<div class="row mb-4">  
  ...
  <div class="col-md-4">
    <div class="form-outline">
      <input type="number" id="days_remaining" name="days_remaining" value="{{isset($requisition) ? $requisition->days_remaining : ''}}" class="form-control" readonly/>
      <label class="form-label" for="days_remaining">DIAS FALTANTES</label>
    </div>
  </div>
</div>
...
<div class="row mb-4 col-md-6">
  <button type="button" @click="submitForm" class="btn btn-primary btn-block col-md-3 m-1">
    {{$modo}} Requisición
</button>
