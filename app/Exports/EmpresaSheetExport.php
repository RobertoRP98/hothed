...
{
    // Obtenemos las facturas relacionadas con la empresa
    $bills = $this->empresa->bills;
    ...
    // Convertimos la colección en un array puro y formateamos los datos
    return $bills->map(function ($bill) {
        ...
        return [
            ...
            'Fecha Factura' => $bill->bill_date ? Carbon::parse($bill->bill_date)->format('d-m-Y') : 'SIN FECHA',
            'Fecha de entrada' => $bill->entry_date ? Carbon::parse($bill->entry_date)->format('d-m-Y') : 'SIN FECHA',
            'Fecha de expiración' => $bill->expiration_date ? Carbon::parse($bill->expiration_date)->format('d-m-Y') : 'SIN FECHA',
            'dias_vencidos_o_por_vencer' => $daysRemaining,
            'descripcion' => $bill->description,
            'total_a_pagar' => $bill->total_payment,
            'status' => $bill->status,
            'fecha_de_pago' => $bill->payment_day ? Carbon::parse($bill->payment_day)->format('d-m-Y') : 'SIN FECHA',
            'comentario' => $bill->comentary,
        ];
    })->toArray(); // Convertimos la colección en un array puro
}
    ...
    public function columnFormats(): array
    {
        return [
            //FECHAS
            'C' => NumberFormat::FORMAT_DATE_DDMMYYYY, // Fecha de Factura
            'D' => NumberFormat::FORMAT_DATE_DDMMYYYY, // Fecha de Entrada
            'E' => NumberFormat::FORMAT_DATE_DDMMYYYY, // Fecha de Expiración
            'J' => NumberFormat::FORMAT_DATE_DDMMYYYY, // Fecha de Pago
            'H' => '"$"#,##0.00_-', // Monto con símbolo de moneda
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Aplicar formato explícito de fecha para las columnas de fechas
                $sheet->getStyle('C7:C' . $sheet->getHighestRow())->getNumberFormat()->setFormatCode('dd/mm/yyyy');
                $sheet->getStyle('D7:D' . $sheet->getHighestRow())->getNumberFormat()->setFormatCode('dd/mm/yyyy');
                $sheet->getStyle('E7:E' . $sheet->getHighestRow())->getNumberFormat()->setFormatCode('dd/mm/yyyy');
                $sheet->getStyle('J7:J' . $sheet->getHighestRow())->getNumberFormat()->setFormatCode('dd/mm/yyyy');
